...
import { getPostCategoriesAPI } from 'services/api';
...
const BlogFilters = ({ onCategoryChange, onSearch, searchValue }: BlogFiltersProps) => {
    ...
    const DesktopFilters = () => (
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
            <div className="flex items-center gap-4">
                <h2 className="text-3xl font-bold font-montserrat text-forest mb-4 md:mb-0">Chia sẻ kinh nghiệm</h2>
                {isAuthenticated && user?.role === 'user' && (
                    <Link to="/my-blog/write">
                        <button className="bg-campfire text-white px-4 py-2 rounded-md font-montserrat flex items-center gap-2 hover:bg-campfire-hover transition-all duration-300">
            ...
            <div className="flex flex-wrap items-center gap-4">
                ...
                <DropdownDefault
                    options={[
                        { _id: 'latest', text: 'Mới nhất' },
                        { _id: 'oldest', text: 'Cũ nhất' },
                        { _id: 'most_viewed', text: 'Xem nhiều nhất' },
                    ]}
